...
@section('content-header')
    <h1>{{ $server->name }}<small>Modifier les informations de ce serveur, notamment son propriétaire et son conteneur.</small></h1>
    <ol class="breadcrumb">
            ...
            <form action="{{ route('admin.servers.view.details', $server->id) }}" method="POST">
                <div class="box-body">
                    <div class="form-group">
                        ...
                        <p class="text-muted small">Caractères autorisés : <code>a-zA-Z0-9_-</code> et <code>[Espace]</code>.</p>
                    </div>
                    <div class="form-group">
                        ...
                        <p class="text-muted small">Laissez ce champ vide pour ne pas attribuer d’identifiant externe à ce serveur. L’identifiant externe doit être unique et ne doit être utilisé par aucun autre serveur.</p>
                    </div>
                    <div class="form-group">
                        ...
                        <p class="text-muted small">Vous pouvez changer le propriétaire de ce serveur en sélectionnant l’adresse e-mail d’un autre utilisateur du système. Un nouveau jeton de sécurité du daemon sera alors généré automatiquement.</p>
                    </div>
                    <div class="form-group">
                ...
                <div class="box-footer">
                    {!! csrf_field() !!}
                    {!! method_field('PATCH') !!}
                    <input type="submit" class="btn btn-sm btn-primary" value="Enregistrer les modifications" />
                </div>
            </form>
        </div>
    </div>
    ...
    $('#pUserId').select2({
        ...
        templateResult: function (data) {
            ...
            return '<div class="user-block"> \
                <img class="img-circle img-bordered-xs" src="https://www.gravatar.com/avatar/' + escapeHtml(data.md5) + '?s=120" alt="Image utilisateur"> \
                <span class="username"> \
        ...
        templateSelection: function (data) {
            ...
            return '<div> \
                <span> \
                    <img class="img-rounded img-bordered-xs" src="https://www.gravatar.com/avatar/' + escapeHtml(data.md5) + '?s=120" style="height:28px;margin-top:-4px;" alt="Image utilisateur"> \
                </span> \
