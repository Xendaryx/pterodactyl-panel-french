<?php

return [
    'exceptions' => [
        'delete_self' => 'Vous ne pouvez pas supprimer votre propre compte.',
        'user_has_servers' => 'Impossible de supprimer un utilisateur ayant des serveurs actifs associés à son compte. Veuillez supprimer ses serveurs avant de continuer.',
    ],
    'notices' => [
        'account_created' => 'Le compte a été créé avec succès.',
        'account_updated' => 'Le compte a été mis à jour avec succès.',
    ],
];
