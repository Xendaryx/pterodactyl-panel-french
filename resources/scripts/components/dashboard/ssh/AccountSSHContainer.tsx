...
export default () => {
    ...
    return (
        <PageContentBlock title={'Clés SSH'}>
            <FlashMessageRender byKey={'account'} />
            <div css={tw`md:flex flex-nowrap my-10`}>
                <ContentBox title={'Ajouter une clé SSH'} css={tw`flex-none w-full md:w-1/2`}>
                    <CreateSSHKeyForm />
                </ContentBox>
                <ContentBox title={'Clés SSH'} css={tw`flex-1 overflow-hidden mt-8 md:mt-0 md:ml-8`}>
                    <SpinnerOverlay visible={!data && isValidating} />
                    {!data || !data.length ? (
                        <p css={tw`text-center text-sm`}>
                            {!data ? 'Chargement...' : 'Aucune clé SSH n’existe pour ce compte.'}
                        </p>
                    ) : (
                        data.map((key, index) => (
                            <GreyRowBox
                            ...
                            >
                                ...
                                <div css={tw`flex-1`}>
                                    ...
                                    <p css={tw`text-xs mt-1 text-neutral-300 uppercase`}>
                                        Added on:&nbsp;
                                        {format(key.createdAt, "dd/MM/yyyy 'à' HH:mm")}
                                    </p>
                                </div>
