            ...
            <div className={'col-span-10 sm:col-span-9 flex'}>
                <div className={'flex-1 px-4 sm:px-0'}>
                    <div className={'flex items-center text-gray-50'}>
                        <Tooltip placement={'top'} content={actor?.email || 'Utilisateur système'}>
                            <span>{actor?.username || 'Système'}</span>
                        </Tooltip>
                        ...
                        <div className={classNames(style.icons, 'group-hover:text-gray-300')}>
                            {activity.isApi && (
                                <Tooltip placement={'top'} content={'Utilisation d’une clé API'}>
                                    <TerminalIcon />
                                </Tooltip>
                            )}
                            {activity.event.startsWith('server:sftp.') && (
                                <Tooltip placement={'top'} content={'Utilisation de SFTP'}>
                                    <FolderOpenIcon />
                                </Tooltip>
                            )}
                            {children}
                    ...
                    <div className={'mt-1 flex items-center text-sm'}>
                        ...
                        <Tooltip placement={'right'} content={format(activity.timestamp, "dd/MM/yyyy 'à' HH:mm:ss")}>
                            <span>{formatDistanceToNowStrict(activity.timestamp, { addSuffix: true, locale: fr })}</span>
                        </Tooltip>
                    </div>
                </div>
