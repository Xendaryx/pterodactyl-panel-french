...
export default ({ database, className }: Props) => {
    ...
    const schema = object().shape({
        confirm: string()
            ...
            .oneOf([database.name.split('_', 2)[1], database.name], 'Le nom de la base de données doit être renseigné.'),
    });
    ...
    return (
        <>
            <Formik onSubmit={submit} initialValues={{ confirm: '' }} validationSchema={schema} isInitialValid={false}>
                {({ isSubmitting, isValid, resetForm }) => (
                    ...
                    >
                        ...
                        <p css={tw`text-sm`}>
                            La suppression d’une base de données est définitive et ne peut pas être annulée. Cette action supprimera définitivement
                            la base de données <strong>{database.name}</strong> ainsi que toutes les données associées.
                        </p>
                        <Form css={tw`m-0 mt-6`}>
                            <Field
                                ...
                                description={'Saisissez le nom de la base de données pour confirmer sa suppression.'}
                            />
            ...
            <Modal visible={connectionVisible} onDismissed={() => setConnectionVisible(false)}>
                ...
                <h3 css={tw`mb-6 text-2xl`}>Informations de connexion à la base de données</h3>
                <div>
                    <Label>Point de connexion</Label>
            ...
            <GreyRowBox $hoverable={false} className={className} css={tw`mb-2`}>
                ...
                <div css={tw`ml-8 text-center hidden md:block`}>
                    ...
                    <p css={tw`mt-1 text-2xs text-neutral-500 uppercase select-none`}>Point de connexion</p>
                </div>
                <div css={tw`ml-8 text-center hidden md:block`}>
