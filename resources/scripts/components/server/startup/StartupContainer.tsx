...
const StartupContainer = () => {
    ...
    return !data ? (
        ...
        <ServerContentBlock title={'Paramètres de démarrage'} showFlashKey={'startup:image'}>
            <div css={tw`md:flex`}>
                <TitledGreyBox title={'Commande de démarrage'} css={tw`flex-1`}>
                    ...
                    {Object.keys(data.dockerImages).length > 1 && !isCustomImage ? (
                        <>
                            ...
                            <p css={tw`text-xs text-neutral-300 mt-2`}>
                                Cette fonctionnalité avancée vous permet de sélectionner une image Docker à utiliser lors de
                                l’exécution de cette instance de serveur.
                            </p>
                        </>
                    ) : (
                        <>
                            ...
                            {isCustomImage && (
                                <p css={tw`text-xs text-neutral-300 mt-2`}>
                                    L’image Docker de ce serveur a été définie manuellement par un administrateur et ne peut
                                    pas être modifiée depuis cette interface.
                                </p>
                            )}
                        </>
                    )}
            ...
            <h3 css={tw`mt-8 mb-2 text-2xl`}>Variables de démarrage</h3>
