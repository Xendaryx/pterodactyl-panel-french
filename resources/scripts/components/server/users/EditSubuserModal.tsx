    ...
    return (
        <Formik
            ...
            validationSchema={object().shape({
                email: string()
                    .max(191, 'Les adresses e-mail ne doivent pas dépasser 191 caractères.')
                    .email('Une adresse e-mail valide doit être renseignée.')
                    .required('Une adresse e-mail valide doit être renseignée.'),
            ...
            <Form>
                <div css={tw`flex justify-between`}>
                    <h2 css={tw`text-2xl`} ref={ref}>
                        {subuser
                            ? `${canEditUser ? 'Modifier les' : 'Voir les'} permissions de ${subuser.email}`
                            : 'Créer un nouvel utilisateur secondaire'}
                    </h2>
                    <div>
                        <Button type={'submit'} css={tw`w-full sm:w-auto`}>
                            {subuser ? 'Enregistrer' : 'Inviter l’utilisateur'}
                        </Button>
                    </div>
                </div>
                ...
                {!isRootAdmin && loggedInPermissions[0] !== '*' && (
                    <div css={tw`mt-4 pl-4 py-2 border-l-4 border-cyan-400`}>
                        <p css={tw`text-sm text-neutral-300`}>
                            Seules les permissions actuellement attribuées à votre compte peuvent être sélectionnées lors de
                            la création ou de la modification d’autres utilisateurs.
                        </p>
                    </div>
                )}
                {!subuser && (
                    <div css={tw`mt-6`}>
                        <Field
                            ...
                            description={
                                'Saisissez l’adresse e-mail de l’utilisateur que vous souhaitez inviter comme utilisateur secondaire de ce serveur.'
                            }
                        />
                    </div>
                )}
