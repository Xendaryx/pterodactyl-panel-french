...
const getActionDetails = (action: string): [string, any] => {
    switch (action) {
        case 'command':
            return ['Envoyer une commande', faCode];
        case 'power':
            return ['Envoyer une action d’alimentation', faToggleOn];
        case 'backup':
            return ['Créer une sauvegarde', faFileArchive];
        default:
            return ['Action inconnue', faCode];
    }
};
        ...
        <div css={tw`sm:flex items-center p-3 sm:p-6 border-b border-neutral-800`}>
            ...
            <div css={tw`flex-none sm:flex-1 w-full sm:w-auto overflow-x-auto`}>
                ...
                {task.payload && (
                    <div css={tw`md:ml-6 mt-2`}>
                        {task.action === 'backup' && (
                            <p css={tw`text-xs uppercase text-neutral-400 mb-1`}>Fichiers et dossiers ignorés :</p>
                        )}
                        <div
            ...
            <div css={tw`mt-3 sm:mt-0 flex items-center w-full sm:w-auto`}>
                ...
                <Can action={'schedule.update'}>
                    <button
                        type={'button'}
                        aria-label={'Modifier la tâche planifiée'}
                ...
                <Can action={'schedule.update'}>
                    <button
                        type={'button'}
                        aria-label={'Supprimer la tâche planifiée'}
