...
export default () => {
    ...
    return (
        <PageContentBlock title={'Planifications'}>
                ...
                <>
                    ...
                    <div css={tw`rounded shadow`}>
                        ...
                        >
                            <div css={tw`flex-1`}>
                                ...
                                <p css={tw`mt-1 text-sm text-neutral-200`}>
                                    Dernière exécution :&nbsp;
                                    {schedule.lastRunAt ? (
                                        format(schedule.lastRunAt, "dd/MM/yyyy 'à' HH:mm")
                                    ) : (
                                        <span css={tw`text-neutral-300`}>N/D</span>
                                    )}
                                    <span css={tw`ml-4 pl-4 border-l-4 border-neutral-600 py-px`}>
                                        Prochaine exécution :&nbsp;
                                        {schedule.nextRunAt ? (
                                            format(schedule.nextRunAt, "dd/MM/yyyy 'à' HH:mm")
                                        ) : (
                                            <span css={tw`text-neutral-300`}>N/D</span>
                                        )}
                                    </span>
                        ...
                        <div css={tw`hidden sm:grid grid-cols-5 md:grid-cols-5 gap-4 mb-4 mt-4`}>
                            ...
                            <CronBox title={'Heure'} value={schedule.cron.hour} />
                            <CronBox title={'Jour du mois'} value={schedule.cron.dayOfMonth} />
                            <CronBox title={'Mois'} value={schedule.cron.month} />
                            <CronBox title={'Jour de la semaine'} value={schedule.cron.dayOfWeek} />
                        </div>
