...
export default () => {
    ...
    const connect = (uuid: string) => {
        ...
        socket.on('SOCKET_CONNECT_ERROR', () => {
            setError('Impossible de se connecter à l’instance WebSocket après plusieurs tentatives : essayez d’actualiser la page.');
        });
        ...
        socket.on('jwt error', (error: string) => {
            ...
            if (reconnectErrors.find((v) => error.toLowerCase().indexOf(v) >= 0)) {
                ...
                setError(
                    'Une erreur est survenue lors de la validation des identifiants fournis pour le WebSocket. Actualisez la page.'
                );
            }
        });
