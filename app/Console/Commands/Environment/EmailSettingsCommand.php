...
class EmailSettingsCommand extends Command
{
    ...
    protected $description = 'Définit ou met à jour la configuration d’envoi des e-mails du Panel.';

    protected $signature = 'p:environment:mail
                            {--driver= : Le service de messagerie à utiliser.}
                            {--email= : L’adresse e-mail utilisée comme expéditeur des messages du Panel.}
                            {--from= : Le nom d’expéditeur affiché pour les e-mails envoyés par le Panel.}
                            {--encryption=}
                            {--host=}
                            {--port=}
                            {--endpoint=}
    ...
    public function handle()
    {
        $this->variables['MAIL_DRIVER'] = $this->option('driver') ?? $this->choice(
            ...
            [
                'smtp' => 'Serveur SMTP',
                'sendmail' => 'Binaire sendmail',
                'mailgun' => 'Messagerie transactionnelle Mailgun',
                'mandrill' => 'Messagerie transactionnelle Mandrill',
                'postmark' => 'Messagerie transactionnelle Postmark',
            ],
        ...
        $this->line('Mise à jour du fichier de configuration d’environnement enregistré.');
        $this->line('');
    }
