...
abstract class EloquentRepository extends Repository implements RepositoryInterface
{
    ...
    {
        Assert::notEmpty($column, 'Le premier argument transmis à updateWhereIn doit être une chaîne non vide.');
    ...
    public function updateOrCreate(array $where, array $fields, bool $validate = true, bool $force = false): Model|bool
    {
        foreach ($where as $item) {
            Assert::true(is_scalar($item) || is_null($item), 'Le premier argument transmis à updateOrCreate doit être un tableau de valeurs scalaires ou nulles ; une valeur de type tableau a été reçue : %s.');
        }
