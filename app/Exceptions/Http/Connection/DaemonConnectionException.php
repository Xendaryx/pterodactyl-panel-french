...
class DaemonConnectionException extends DisplayException
{
    ...
    public function __construct(GuzzleException $previous, bool $useStatusCode = true)
    {
        ...
        if (is_null($response)) {
            $message = 'Impossible d’établir une connexion avec la machine exécutant ce serveur. Veuillez réessayer.';
        } else {
            $message = sprintf('Une erreur est survenue lors de la communication avec la machine exécutant ce serveur. Cette erreur a été enregistrée. Veuillez réessayer. (code : %s) (request_id : %s)', $response->getStatusCode(), $this->requestId ?? '<nil>');
        }
        ...
        if ($this->statusCode < 500 && !is_null($response)) {
            ...
            $message = sprintf('Une erreur est survenue sur l’hôte distant : %s. (ID de requête : %s)', $body['error'] ?? $message, $this->requestId ?? '<nil>');
        }
