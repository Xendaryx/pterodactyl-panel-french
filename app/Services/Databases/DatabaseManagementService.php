...
class DatabaseManagementService
{
    ...
    public function create(Server $server, array $data): Database
    {
        ...
        if (empty($data['database']) || !preg_match(self::MATCH_NAME_REGEX, $data['database'])) {
            throw new \InvalidArgumentException('Le nom de la base de données transmis à DatabaseManagementService::handle DOIT être préfixé par "s{server_id}_".');
        }
    ...
    protected function createModel(array $data): Database
    {
        ...
        if ($exists) {
            throw new DuplicateDatabaseNameException('Une base de données portant ce nom existe déjà pour ce serveur.');
        }
