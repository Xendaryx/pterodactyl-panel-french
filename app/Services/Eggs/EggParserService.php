...
class EggParserService
{
    ...
    public function handle(UploadedFile $file): array
    {
        if ($file->getError() !== UPLOAD_ERR_OK || !$file->isFile()) {
            throw new InvalidFileUploadException('Le fichier sélectionné n’est pas valide et ne peut pas être importé.');
        }
        ...
        if (!in_array(Arr::get($parsed, 'meta.version') ?? '', ['PTDL_v1', 'PTDL_v2'])) {
            throw new InvalidFileUploadException('Le fichier JSON fourni n’est pas dans un format reconnu.');
        }
