        ...
        $server = $this->connection->transaction(function () use ($server, $data) {
            ...
            if (isset($data['allocation_id']) && $data['allocation_id'] != $server->allocation_id) {
                try {
                    ...
                    throw new DisplayException('L’allocation principale demandée n’est actuellement pas attribuée à ce serveur.');
                }
            }
            ...
            foreach ($data['remove_allocations'] as $allocation) {
                ...
                if ($allocation === ($data['allocation_id'] ?? $server->allocation_id)) {
                    if (empty($freshlyAllocated)) {
                        throw new DisplayException('Vous tentez de supprimer l’allocation principale de ce serveur, mais aucune allocation de remplacement n’est disponible.');
                    }
