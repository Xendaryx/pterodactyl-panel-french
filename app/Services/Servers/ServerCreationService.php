...
class ServerCreationService
{
    ...
    public function handle(array $data, ?DeploymentObject $deployment = null): Server
    {
        ...
        if (empty($data['node_id'])) {
            Assert::false(empty($data['allocation_id']), 'Une valeur allocation_id non vide est attendue dans les données de création du serveur.');

            $data['node_id'] = Allocation::query()->findOrFail($data['allocation_id'])->node_id;
        }

        if (empty($data['nest_id'])) {
            Assert::false(empty($data['egg_id']), 'Une valeur egg_id non vide est attendue dans les données de création du serveur.');
