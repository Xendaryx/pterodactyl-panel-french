...
class BackupStatusController extends Controller
{
    ...
    public function index(ReportBackupCompleteRequest $request, string $backup): JsonResponse
    {
        ...
        if ($model->is_successful) {
            throw new BadRequestHttpException('Impossible de modifier l’état d’une sauvegarde déjà marquée comme terminée.');
        }
    ...
    protected function completeMultipartUpload(Backup $backup, S3Filesystem $adapter, bool $successful, ?array $parts): void
    {
        ...
        if (empty($backup->upload_id)) {
            ...
            throw new DisplayException('Impossible de terminer la demande de sauvegarde : aucun upload_id n’est présent dans le modèle.');
        }
