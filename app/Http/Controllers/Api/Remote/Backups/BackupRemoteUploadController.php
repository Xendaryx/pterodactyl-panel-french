...
class BackupRemoteUploadController extends Controller
{
    ...
    public function __invoke(Request $request, string $backup): JsonResponse
    {
        ...
        if (empty($size)) {
            throw new BadRequestHttpException('Le paramètre de requête "size" doit être renseigné et ne peut pas être vide.');
        }
        ...
        if ($server->node_id !== $node->id) {
            throw new HttpForbiddenException('Le nœud à l’origine de la requête n’est pas autorisé à accéder à ce serveur.');
        }

        // Prevent backups that have already been completed from trying to
        // be uploaded again.
        if (!is_null($model->completed_at)) {
            throw new ConflictHttpException('Cette sauvegarde est déjà dans un état terminé.');
        }

        // Ensure we are using the S3 adapter.
        $adapter = $this->backupManager->adapter();
        if (!$adapter instanceof S3Filesystem) {
            throw new BadRequestHttpException('L’adaptateur de sauvegarde configuré n’est pas compatible avec S3.');
        }
