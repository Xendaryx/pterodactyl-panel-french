...
class SftpAuthenticationController extends Controller
{
    ...
    public function __invoke(SftpAuthenticationFormRequest $request): JsonResponse
    {
        ...
        if (empty($connection['server'])) {
            throw new BadRequestHttpException('Aucun identifiant de serveur valide n’a été fourni dans la requête.');
        }

        if ($this->hasTooManyLoginAttempts($request)) {
            $seconds = $this->limiter()->availableIn($this->throttleKey($request));

            throw new TooManyRequestsHttpException($seconds, "Trop de tentatives de connexion pour ce compte. Veuillez réessayer dans $seconds secondes.");
        }
    ...
    protected function reject(Request $request, bool $increment = true): void
    {
        ...
        throw new HttpForbiddenException('Les identifiants d’autorisation sont incorrects. Veuillez réessayer.');
    }
    ...
    protected function validateSftpAccess(User $user, Server $server): void
    {
        if (!$user->root_admin && $server->owner_id !== $user->id) {
            ...
            if (!in_array(Permission::ACTION_FILE_SFTP, $permissions)) {
                ...
                throw new HttpForbiddenException('Vous n’avez pas l’autorisation d’accéder au SFTP de ce serveur.');
            }
        }
