...
class TwoFactorController extends ClientApiController
{
    ...
    public function index(Request $request): JsonResponse
    {
        if ($request->user()->use_totp) {
            throw new BadRequestHttpException('L’authentification à deux facteurs est déjà activée sur ce compte.');
        }
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        if (!password_verify($data['password'], $request->user()->password)) {
            throw new BadRequestHttpException('Le mot de passe fourni est incorrect.');
        }
