...
class BackupController extends ClientApiController
{
    ...
    public function download(Request $request, Server $server, Backup $backup): JsonResponse
    {
        ...
        if ($backup->disk !== Backup::ADAPTER_AWS_S3 && $backup->disk !== Backup::ADAPTER_WINGS) {
            throw new BadRequestHttpException('La sauvegarde demandée utilise un type de stockage inconnu et ne peut pas être téléchargée.');
        }
    ...
    public function restore(RestoreBackupRequest $request, Server $server, Backup $backup): JsonResponse
    {
        ...
        if (!is_null($server->status)) {
            throw new BadRequestHttpException('Ce serveur n’est actuellement pas dans un état permettant la restauration d’une sauvegarde.');
        }

        if (!$backup->is_successful || is_null($backup->completed_at)) {
            throw new BadRequestHttpException('Cette sauvegarde ne peut pas être restaurée pour le moment : elle n’est pas terminée ou a échoué.');
        }
