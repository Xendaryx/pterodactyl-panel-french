...
class NetworkAllocationController extends ClientApiController
{
    ...
    public function store(NewAllocationRequest $request, Server $server): array
    {
        $allocation = Activity::event('server:allocation.create')->transaction(function ($log) use ($server) {
            if ($server->allocations()->lockForUpdate()->count() >= $server->allocation_limit) {
                throw new DisplayException('Impossible d’attribuer des allocations supplémentaires à ce serveur : la limite a été atteinte.');
            }
    ...
    public function delete(DeleteAllocationRequest $request, Server $server, Allocation $allocation): JsonResponse
    {
        ...
        if (empty($server->allocation_limit)) {
            throw new DisplayException('Vous ne pouvez pas supprimer d’allocations pour ce serveur : aucune limite d’allocation n’est définie.');
        }

        if ($allocation->id === $server->allocation_id) {
            throw new DisplayException('Vous ne pouvez pas supprimer l’allocation principale de ce serveur.');
        }
