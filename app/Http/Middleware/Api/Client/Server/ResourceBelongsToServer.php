...
class ResourceBelongsToServer
{
    ...
    public function handle(Request $request, \Closure $next): mixed
    {
        ...
        if (! isset($params['server']) || !$params['server'] instanceof Server) {
            throw new \InvalidArgumentException('Ce middleware ne peut pas être utilisé dans un contexte où aucun serveur n’est présent dans les paramètres.');
        }

        /** @var Server $server */
        $server = $request->route()->parameter('server');
        $exception = new NotFoundHttpException('La ressource demandée n’a pas été trouvée pour ce serveur.');
        foreach ($params as $key => $model) {
            ...
            switch (get_class($model)) {
                ...
                default:
                    ...
                    throw new \InvalidArgumentException('Aucun gestionnaire n’est configuré pour une ressource de ce type : ' . get_class($model));
            }
        }
