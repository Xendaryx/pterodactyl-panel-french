...
class DaemonAuthenticate
{
    ...
    public function handle(Request $request, \Closure $next): mixed
    {
        ...
        if (is_null($bearer = $request->bearerToken())) {
            throw new HttpException(401, 'L’accès à ce point de terminaison nécessite un en-tête Authorization.', null, ['WWW-Authenticate' => 'Bearer']);
        }

        $parts = explode('.', $bearer);
        // Ensure that all of the correct parts are provided in the header.
        if (count($parts) !== 2 || empty($parts[0]) || empty($parts[1])) {
            throw new BadRequestHttpException('L’en-tête Authorization fourni n’est pas dans un format valide.');
        }
